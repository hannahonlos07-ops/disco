...
// Default lookback for the list when no window is asked for (days).
const DEFAULT_LIST_DAYS = Number(process.env.PANDADOC_LIST_DEFAULT_DAYS ?? "10");
// Tiny PER-PERSON cooldown (ms) just to swallow accidental double-taps — never
// blocks a different person, and clears in a few seconds.
const CONTRACT_COOLDOWN_MS = 3_000;
const contractCooldown = new Map(); // userId -> last-request timestamp

// Parse a time window out of the request — "last 24 hours", "past 5 days",
// "10 days", "last 24 days". Returns {ms, label} or null if none mentioned.
...
// Fetches unsigned contracts within `windowMs` (default 10 days) and returns an
// array of Discord-sized messages (newest first, sleek, name-first).
...
client.on(Events.MessageCreate, async (message) => {
  try {
    ...
    if (CONTRACTS_ENABLED && CONTRACT_LIST_GUILD_IDS.includes(message.guildId) && isUnsignedListRequest(message.content)) {
      const ccKey = message.author.id; // per-person, not global
      if (Date.now() - (contractCooldown.get(ccKey) ?? 0) < CONTRACT_COOLDOWN_MS) return;
      contractCooldown.set(ccKey, Date.now());
      try {
        // Use the window they asked for ("last 5 days", "24 hours"), else default.
